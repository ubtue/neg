...
public class LemmaDB extends AbstractBase {
    ...
    public static List<String> getListErstglied() throws Exception {
        return getStringListNative("SELECT DISTINCT SUBSTRING_INDEX(MGHLemma, '~', 1) AS Erstglied  FROM neg.mgh_lemma WHERE MGHLemma LIKE '%~%' AND MGHLemma NOT LIKE '%"+AbstractBase.escapeSql(Constants.forbiddenLemmaSubstring)+"%' ORDER BY Erstglied ASC");
    }

    public static List<String> getListZweitglied() throws Exception {
        return getStringListNative("SELECT DISTINCT SUBSTRING_INDEX(MGHLemma, '~', -1) AS Zweitglied  FROM neg.mgh_lemma WHERE MGHLemma LIKE '%~%' AND MGHLemma NOT LIKE '%"+AbstractBase.escapeSql(Constants.forbiddenLemmaSubstring)+"%' ORDER BY Zweitglied ASC");
    }
